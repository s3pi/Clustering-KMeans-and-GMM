...
#    plt.scatter(data[:,0], data[:,1], s=5,c = colors[color_id])
    ...
    centers = np.resize(centers, (k, 32))
...
#    plt.scatter(curr_centers[:,0], curr_centers[:,1], marker='*', c='r', s=10)
...
def groupdata(data, centers_after_Kmeans, whcluster, k):
    ...
    return cluster_data
#        for i in range(len(whcluster)):
#            if(whcluster == )
#    for i in range(k):
#       cluster_data[i] = data[whcluster == i]
#    return cluster_data
    
def contour_plot(data, centers, cluster_d, k, tot_rows):
    cat_color = (u'#FFAFAF', u'#BBFFB9', u'#BBB9FF', u'#ffff00')
    colors = (u'#ff0000', u'#228b22', u'#0000cd', u'#FF8000', u'#B23AEE', u'#8FBC8F', u'#BCEE68', u'#00EEEE', u'#EE6A50', u'#838B8B', u'#CDAA7D', u'#1C86EE', u'#8B3E2F',u'#6495ED',u'#8B8878',u'#00BFFF')
    #Colors are Red1, forestgreen, blue3,orange, darkorchid2,
    #darkseagreen,darkolivegreen2,cyan2,azure4,burlywood3,dodgerblue2,coral4,cornflowerblue,cornsilk4 ,deepskyblue1 
...
def main():
    ...
    if(opt_type == 1):
        ...
        data_all_classes_test = load_scene_image_test_set()
    elif opt_type == 4:
    ...
    for i in range(btw_num_of_classes):
        ...
        centers_after_Kmeans, whcluster = computeKMC(data_some_classes[i], centers, k)
    ...
    confusion_matrix = compute_confusion_matrix(data_all_classes_test, pi_each_class_each_k, mu_each_class_each_k, sigma_each_class_each_k, k)
    print("\nconfusion_matrix")
...
def test_which_class(x, pi_each_class_each_k, mu_each_class_each_k, sigma_each_class_each_k, k):
    prob_xi_each_class = []
    for i in range(3):
        summation = 0
        for j in range(k):
...
def compute_GMM(centers_after_Kmeans, data, cluster_data, k):
    ...
    while abs(log_likelihood_new - log_likelihood_old) > 0.01:
...
def compute_prob_each_Xn_each_k_1(data, mu_each_k, pi_each_k, sigma_each_k, k):
    ...
    for i in range(k):
        nix = []
        for Xi in data:
            nix.append(norm_pdf_multivariate(Xi, mu_each_k[i], sigma_each_k[0][i]))
        nix = np.asarray(nix)
        prob_each_Xn_each_k[:, i] = pi_each_k[i] * nix #probability for each data point belonging to cluster k
    return prob_each_Xn_each_k
...
def compute_init_sigma(data, cluster_data, k):
    ...
    for i in range(k):
        sigma_init_each_k[0][i] = np.cov(cluster_data[i].T)
        for p in range(24):
            for q in range(24):
                if p != q:
